...
void ADC(void) 
{ 
    OpenADC( ADC_FOSC_32 & ADC_RIGHT_JUST & ADC_8_TAD, ADC_CH1 & ADC_INT_OFF & ADC_REF_VDD_VSS,0);//ADC channel 1 is initialised
	OpenADC( ADC_FOSC_32 & ADC_RIGHT_JUST & ADC_8_TAD, ADC_CH2 & ADC_INT_OFF & ADC_REF_VDD_VSS,0);//ADC channel 2 is initialised
	OpenADC( ADC_FOSC_32 & ADC_RIGHT_JUST & ADC_8_TAD, ADC_CH3 & ADC_INT_OFF & ADC_REF_VDD_VSS,0);//ADC channel 3 is initialised

    //configure USART 
    OpenUSART( USART_TX_INT_OFF & USART_RX_INT_OFF & USART_ASYNCH_MODE & USART_EIGHT_BIT & USART_CONT_RX & USART_BRGH_HIGH,25); // Sensor 1 connecting to ADC CH1/RA1 
            SetChanADC(ADC_CH1); 
            Delay10KTCYx(5); 
            ConvertADC(); 
            while(BusyADC()); 
...
while(1) 
        { 
         ADC(); 
		 Moisture = (float)ADCresult1*0.129; //After calibration	  
         Temperature=-25.445*(float)ADCresult2*3.2/1023+73.169;//After calibration, coefficients change
		 Light=(float)ADCresult3*3.2/1023;
         TxPacket.GID = 23;//Group ID 
         TxPacket.NID = 3;//Node ID 
         TxPacket.Data1 = Moisture;
		 TxPacket.Data2 = Temperature;
		 TxPacket.Data3 = Light;
                 ...
                 switch(RxPacket.NID) //Apply different delay for different nodeID 
                       { 
                        case 2:  
                               ...
                               break; 
                        case 3:  
                               sprintf(Text,"Group ID=%u Node ID=%u Moisture=%.2f Temperature=%.2f C Light=%.2f V  RSSI =%u CRCRight=%u \r\n", //NOTICE: Alterd for moisture sensor
                                                TxPacket.GID, TxPacket.NID, TxPacket.Data1, TxPacket.Data2, TxPacket.Data3, Strength,CRCRight); 
                               USARTOut(Text,strlen(Text)); 
                               break; 
                       } 
